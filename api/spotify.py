...
def makeSVG(data, background_color, border_color):
    # Forzar color de fondo blanco por defecto
    ...
    if not "is_playing" in data:
        try:
            ...
            if "items" not in recentPlays:
                print("Error en la respuesta de Spotify:", recentPlays)
                item = {
                    "album": {"images": [{"url": PLACEHOLDER_URL}]},
                    "artists": [{"name": "No hay música reciente", "external_urls": {"spotify": "#"}}],
                    "name": "No hay música reciente",
            ...
            print("Error al obtener música reciente:", str(e))
            item = {
                "album": {"images": [{"url": PLACEHOLDER_URL}]},
                "artists": [{"name": "Error al cargar", "external_urls": {"spotify": "#"}}],
    ...
    try:
        if not item["album"]["images"]:
            print("No hay imágenes disponibles en el álbum")
            image = PLACEHOLDER_IMAGE
            barPalette = gradientGen(PLACEHOLDER_URL, 4)
            songPalette = gradientGen(PLACEHOLDER_URL, 2)
        else:
            print(f"Número de imágenes disponibles: {len(item['album']['images'])}")
        ...
        print(f"Error al procesar las imágenes: {str(e)}")
        image = PLACEHOLDER_IMAGE
        barPalette = [(0, 0, 0)] * 4
        songPalette = [(0, 0, 0)] * 2
...
def catch_all(path):
    # Forzar un color de fondo blanco por defecto
    background_color = request.args.get('background_color')
    if not background_color or background_color.lower() == 'transparent':
        background_color = "FFFFFF"  # Color blanco por defecto
